...
if (!deviceUserId) {
    deviceUserId = 'user_' + Math.random().toString(36).substring(2, 9) + Date.now().toString(36);
...
async function loadUserWallet() {
    try {
        ...
        if (userSnap.exists() && userSnap.data().balance !== undefined) {
            ...
            await setDoc(userDocRef, { balance: 100 }, { merge: true });
        }
        ...
        let html = `
            <div class="tx-item">
                <div style="display: flex; align-items: center; gap: 10px;">
                    <div class="tx-icon success-icon">📥</div>
                    <div>
                        <div style="font-size: 13px; font-weight: 700; color: white;">Welcome Bonus</div>
                        <div style="font-size: 10px; color: var(--text-muted);">Initial Account Credit</div>
                    </div>
                </div>
                <div style="font-size: 13px; font-weight: 800; color: var(--success);">+🪙 100</div>
            </div>
        `;
...
window.openTournamentDetailsModal = function(title, modeLabel, category, slots, currentJoined, fee, p1, p2, p3, booyahPrize, winnerPrize, totalPrize, perKill, map, scheduleTime, room, id, isJoined) {
    ...
    // Check if Battle Royale category to show "+ Per Kill" text next to ranks
    ...
    if (category === 'br-solo') {
        prizeHtml = `
            <div style="background: #05080f; border: 1px solid var(--card-border); padding: 12px; border-radius: 12px; margin-bottom: 12px;">
    ...
    bodyEl.innerHTML = `
        <div style="max-height: 350px; overflow-y: auto; padding-right: 4px;">
            ...
            <div style="background: rgba(239, 68, 68, 0.08); border: 1px solid rgba(239, 68, 68, 0.2); padding: 12px; border-radius: 12px; margin-bottom: 6px;">
                <div style="font-size: 11px; font-weight: bold; color: var(--danger); margin-bottom: 4px;">⚠️ Rules & Important Notes:</div>
                <ul style="margin: 0; padding-left: 16px; font-size: 11px; color: var(--text-muted); line-height: 1.4;">
                    <li>Hacking or third-party tools = Permanent Ban.</li>
                    <li>Emulators not allowed. Mobile devices only.</li>
                    <li>Join room 5 minutes before match time.</li>
                </ul>
            </div>
        </div>
    `;
    ...
    if(submitBtn) {
        if(isJoined) {
            ...
            submitBtn.style.border = "1px solid var(--success)";
            submitBtn.onclick = () => {
...
window.openJoinModal = async function(id, title, category, fee, maxSlots) {
    ...
    try {
        ...
        if (userSnap.exists()) {
            ...
            currentBalance = Number(userSnap.data().balance) || 100;
        }
    } catch(e) {
    ...
    let formHtml = `
        <p style="font-size: 13px; color: var(--text-muted); margin-bottom: 14px;">Entry Fee: <b style="color: var(--accent);">${fee} Coins</b> | Your Balance: <b style="color: var(--success);">${currentBalance} Coins</b> (${isSquad ? 'Squad - 4 Players' : 'Solo'})</p>
    `;
    ...
    formHtml += `
        <div style="display: flex; gap: 10px;" id="modalBtnContainer">
            <button type="button" id="modalCancelBtn" style="flex: 1; padding: 12px; background: #1e293b; color: var(--text-muted); border: 1px solid var(--card-border); border-radius: 12px; font-weight: bold; cursor: pointer;">Cancel</button>
            <button type="button" id="modalConfirmJoinBtn" style="flex: 1; padding: 12px; background: var(--accent); color: #05080f; border: none; border-radius: 12px; font-weight: bold; cursor: pointer; box-shadow: 0 4px 15px var(--accent-glow);">Join Now</button>
        </div>
    `;
    ...
    document.getElementById('modalConfirmJoinBtn').onclick = async () => {
        if (currentBalance < fee) {
            alert(`Insufficient balance! You need ${fee} Coins to join this match.`);
            return;
        }
        ...
        if (isSquad) {
            ...
            if (!p1 || !p2 || !p3 || !p4 || !contact) {
                alert("Please fill in all 4 players' IDs and contact info!");
                return;
            }
        ...
        try {
            ...
            if (currentFilledSlots + slotsNeeded > maxSlots) {
                alert("Sorry! Not available slots for this entry.");
                modal.classList.add('hidden');
                return;
            }
            ...
            bodyEl.innerHTML = `
                <div style="text-align: center; padding: 15px 0;">
                    <div style="width: 55px; height: 55px; background: var(--success); color: #05080f; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 26px; margin: 0 auto 12px auto;">✓</div>
                    <h4 style="color: var(--text-main); font-size: 16px; margin-bottom: 6px;">Successfully Joined!</h4>
                    <p style="color: var(--text-muted); font-size: 13px; margin-bottom: 10px;">Registered for <b>${title}</b>. (${fee} Coins deducted)</p>
                    <div style="font-weight: bold; color: var(--accent); font-size: 13px;">Awesome, Let's Play! 🚀</div>
                </div>
            `;
    ...
    if (!roomData || roomData === 'Not published yet') {
        bodyEl.innerHTML = `
            <div style="text-align: center; padding: 15px 0;">
                ...
                <h4 style="color: var(--text-main); font-size: 15px; margin-bottom: 6px;">Waiting for Admin</h4>
        ...
        bodyEl.innerHTML = `
            <div style="background: #05080f; border: 1px solid var(--card-border); padding: 14px; border-radius: 12px; margin-bottom: 15px;">
            ...
            <button type="button" id="copyRoomBtn" class="btn" style="background: var(--success); color: #05080f;">📋 Copy Room Details</button>
        `;
        setTimeout(() => {
            ...
            if(copyBtn) {
                copyBtn.onclick = () => {
                    ...
                    setTimeout(() => copyBtn.innerText = "📋 Copy Room Details", 2000);
                };
            }
        }, 100);
    }
    ...
    bodyEl.innerHTML = `
        <div style="max-height: 400px; overflow-y: auto; text-align: center;">
            ...
            <div style="background: white; padding: 10px; border-radius: 12px; display: inline-block; margin-bottom: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.3);">
                <img id="dynamicQrImg" src="${qrImgSrc}" alt="UPI QR Code" style="width: 140px; height: 140px; display: block; object-fit: contain;">
            </div>
            ...
            <div style="background: #05080f; border: 1px solid var(--card-border); padding: 10px; border-radius: 10px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;">
                <span style="font-size: 12px; color: white; word-break: break-all;" id="upiIdText">${upiId}</span>
            ...
            <p style="font-size: 10px; color: var(--text-muted); margin-bottom: 12px; line-height: 1.3;">
                💡 Note: If the direct button shows an error on your device, please scan the QR code or copy the UPI ID to pay from your preferred payment app. Transaction ID (UTR) dalkar submit karein.
            </p>

            <div style="text-align: left; background: #05080f; border: 1px solid var(--card-border); padding: 12px; border-radius: 12px;">
                ...
                <button type="button" class="btn" style="background: var(--accent); color: #05080f; padding: 12px; font-size: 13px;" onclick="submitDepositRequest()">Submit Deposit</button>
            </div>
        </div>
    `;
...
window.submitDepositRequest = async function() {
    ...
    if (!utr) {
        alert("Please enter the UTR / Transaction ID to submit request!");
        return;
    }
    if (amount < 20) {
        alert("Minimum deposit amount is 20 Coins!");
        return;
    }
    ...
    try {
        ...
        const modal = document.getElementById('customModal');
        document.getElementById('modalTitle').innerText = "Request Submitted! 🎉";
        document.getElementById('modalBody').innerHTML = `
            <div style="text-align: center; padding: 20px 0;">
                <div style="font-size: 40px; margin-bottom: 10px;">⏳</div>
                <h4 style="color: white; font-size: 15px; margin-bottom: 6px;">Verification in Progress</h4>
                <p style="color: var(--text-muted); font-size: 13px; margin-bottom: 15px;">Your deposit request of <b>${amount} Coins</b> has been submitted. Coins will be added within 1-2 hours after verification.</p>
                <button type="button" class="btn" style="background: var(--accent); color: #05080f; padding: 10px;" onclick="document.getElementById('customModal').classList.add('hidden')">OK</button>
            </div>
        `;
...
window.openWithdrawModal = function() {
    ...
    bodyEl.innerHTML = `
        <div style="max-height: 350px; overflow-y: auto;">
            <p style="font-size: 12px; color: var(--text-muted); margin-bottom: 14px;">Minimum Withdrawal: <b style="color: var(--accent);">20 Coins (₹20)</b></p>
            ...
            <button type="button" class="btn" style="background: var(--accent); color: #05080f; padding: 12px;" onclick="submitWithdrawRequest()">Submit Request</button>
        </div>
    `;
...
window.submitWithdrawRequest = async function() {
    ...
    if (amount < 20) {
        alert("Minimum withdrawal amount is 20 Coins!");
        return;
    }
    ...
    try {
        ...
        document.getElementById('modalBody').innerHTML = `
            <div style="text-align: center; padding: 20px 0;">
                <div style="font-size: 40px; margin-bottom: 10px;">✅</div>
                <h4 style="color: white; font-size: 15px; margin-bottom: 6px;">Successful Withdrawal Request Submitted</h4>
                <p style="color: var(--text-muted); font-size: 13px; margin-bottom: 15px;">Your payout request of <b>${amount} Coins</b> has been sent to admin for verification.</p>
                <button type="button" class="btn" style="background: var(--accent); color: #05080f; padding: 10px;" onclick="document.getElementById('customModal').classList.add('hidden')">OK</button>
            </div>
        `;
        document.getElementById('modalActions').style.display = 'none';
...
window.openProfileSettingsModal = async function() {
    ...
    bodyEl.innerHTML = `
        ...
        <div style="display: flex; gap: 10px;">
            <button type="button" id="saveProfileBtn" class="btn" style="background: var(--accent); color: #05080f;">Save to Cloud ☁️</button>
        </div>
    `;
    ...
    document.getElementById('saveProfileBtn').onclick = async () => {
        ...
        try {
            ...
            alert("Profile saved to Firebase Cloud successfully! ☁️");
            modal.classList.add('hidden');
        } catch (err) {
        ...
        let html = `
            <div style="max-height: 300px; overflow-y: auto;">
                <div style="background: #05080f; border: 1px solid var(--card-border); padding: 10px 12px; border-radius: 10px; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center; font-size: 12px;">
                    ...
                    <b style="color: var(--success);">+🪙 100</b>
                </div>
        `;
...
window.openReferralModal = function() {
    ...
    let referralCode = "NEXT" + deviceUserId.slice(-4).toUpperCase();
...
window.openTelegramSupport = function() {
    window.open(`https://t.me/AapkaTelegramUsername`, '_blank');
};

// --- Admin Panel & Banner URL Manager ---
...
window.approveDeposit = async function(depId, targetUserId, amount, utr) {
    try {
        ...
        alert(`Deposit approved! ${amount} coins added to user wallet.`);
        loadAdminTournaments();
        loadUserWallet();
        loadUserTransactions();
    } catch(e) {
...
window.approveWithdrawal = async function(witId, targetUserId, amount, upi) {
    try {
        ...
        alert(`Withdrawal approved! ${amount} coins deducted from user wallet.`);
        loadAdminTournaments();
        loadUserWallet();
        loadUserTransactions();
    } catch(e) {
    ...
    try {
        ...
        if (players.length === 0) {
            ...
            if(submitBtn) {
                ...
                submitBtn.onclick = () => modal.classList.add('hidden');
            }
...
async function addPrizeToUserWallet(targetUserId, amount, playerName) {
    try {
        ...
        alert(`Successfully added 🪙 ${amount} coins to ${playerName}'s wallet!`);
        loadUserWallet();
        loadUserTransactions();
    } catch (e) {
...
window.publishRoom = async function(id) {
    ...
    try {
        ...
        alert("Error updating room info: " + e.message);
    }
};
